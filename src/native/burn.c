/**
 * Scatter — Native pthread CPU/memory burn.
 *
 * Compiled at runtime via Bun's built-in `cc` (bun:ffi) or pre-compiled
 * as a shared library (.dylib / .so). Exposes three symbols:
 *
 *   get_hw_threads()      → int32   — number of online HW threads (cgroup-aware on Linux)
 *   result_struct_size()   → int32   — sizeof(ThreadResult) for buffer allocation
 *   native_burn(...)       → int32   — spawn N pthreads, burn CPU, fill memory
 *
 * The Leibniz series for π is used as the CPU workload because it is:
 *   • Pure arithmetic (no I/O, no syscalls, no branching except loop)
 *   • Deterministic (same result regardless of duration)
 *   • Impossible to optimise away (result is consumed)
 */
...
/* ------------------------------------------------------------------ */
/* Result struct — 32 bytes, naturally aligned for double              */
/* ------------------------------------------------------------------ */

typedef struct {
    double   pi;           /* offset  0 — computed π approximation  */
    int64_t  iterations;   /* offset  8 — total Leibniz iterations  */
    int32_t  thread_id;    /* offset 16 — which thread produced it  */
    int32_t  alloc_mb;     /* offset 20 — memory actually allocated */
    double   elapsed_ms;   /* offset 24 — wall-clock time this thread ran */
} ThreadResult;
...
static void *burn_thread(void *arg) {
    ...
    work->result->alloc_mb   = alloc_mb;
    work->result->elapsed_ms = elapsed;
...
int32_t native_burn(int32_t num_threads,
                    ...
                    void   *results_ptr)
{
    if (num_threads <= 0 || !results_ptr) return -1;
